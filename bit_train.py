...
def main():
    ...
    f_path = os.path.join(FLAGS.save_path, str(today_))
    print(f_path)
    ...
    if FLAGS.train:
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            with tf.variable_scope("model", reuse=None):
                m_tr = BitLstmOModel(is_training=True, config=model_config)
            with tf.variable_scope("model", reuse=True):
                m_va = BitLstmOModel(is_training=False, config=model_config)
